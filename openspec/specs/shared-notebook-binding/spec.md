...
#### Scenario: 提交 Google 分享頁產生的 Notebook 網址
- **WHEN** 已授權學員提交 `https://notebook.google.com/notebook/<id>` 分享網址
- **THEN** 系統原樣接受網址、擷取相同 Notebook ID 並進入存取驗證
- **THEN** 學員不需要將網址手動改寫為舊版 NotebookLM 網域
...
#### Scenario: 拒絕偽造官方網域
- **WHEN** 學員提交僅以官方名稱為前綴或子網域的網址，例如 `notebook.google.com.evil.example`
- **THEN** 系統以精確 host allowlist 拒絕請求且不修改既有綁定
