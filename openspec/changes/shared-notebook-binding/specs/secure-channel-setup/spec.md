...
### Requirement: 管理員可分頁瀏覽學員與邀請碼清單
系統 SHALL 將學員綁定清單與未使用邀請碼清單分別以每頁最多 10 筆呈現，並在資料超過 10 筆時提供頁碼切換；分頁不得遺漏或改變清單資料的既有排序。

#### Scenario: 學員清單超過十筆
- **WHEN** 學員綁定清單依既有排序包含 11 筆以上資料
- **THEN** 第一頁顯示排序後第 1 至 10 筆，第二頁顯示第 11 筆開始的資料
- **THEN** 管理員可使用上一頁、下一頁或頁碼選擇瀏覽所有學員

#### Scenario: 邀請碼清單不超過十筆
- **WHEN** 未使用邀請碼清單包含 10 筆或更少資料
- **THEN** 系統顯示全部邀請碼
- **THEN** 系統可隱藏不必要的分頁控制，但不得隱藏任何資料

#### Scenario: 分頁中的資料被刪除
- **WHEN** 管理員刪除目前最後一頁的最後一筆邀請碼
- **THEN** 系統重新載入清單並將頁碼限制在仍有資料的最後一頁

### Requirement: 管理員可複製及刪除未使用邀請碼
系統 SHALL 在未使用邀請碼旁提供複製與刪除操作；複製 SHALL 將完整邀請碼寫入瀏覽器剪貼簿，刪除 MUST 僅允許 `used=0` 的邀請碼，且 MUST 由管理員工作階段授權。

#### Scenario: 複製未使用邀請碼
- **WHEN** 管理員按下某筆未使用邀請碼旁的「複製」
- **THEN** 系統將該筆完整邀請碼寫入剪貼簿
- **THEN** 畫面顯示短暫的複製成功狀態，且不改變邀請碼資料

#### Scenario: 刪除未使用邀請碼
- **WHEN** 管理員確認刪除一筆 `used=0` 的邀請碼
- **THEN** 系統刪除該筆邀請碼並從清單移除
- **THEN** 系統不得刪除任何 Channel、Notebook 綁定或其他邀請碼

#### Scenario: 阻擋已使用邀請碼刪除
- **WHEN** 管理員嘗試刪除 `used=1` 的邀請碼，或刪除前該邀請碼已被使用
- **THEN** 系統回傳狀態衝突並保留該邀請碼
- **THEN** 系統不得刪除其 Channel、Notebook 綁定或設定工作階段
