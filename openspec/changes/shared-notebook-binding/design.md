...
- 讓管理者在學員數量增加時仍能以每頁 10 筆的分頁清單找到所有資料，並能快速複製或清理未使用邀請碼。
...
學員綁定清單與未使用邀請碼清單各自獨立分頁，每頁固定呈現 10 筆，並提供上一頁、下一頁與頁碼選擇。清單沿用後端目前的建立時間排序；第一頁是排序後第 1 至 10 筆，第二頁是第 11 至 20 筆。資料不超過 10 筆時可隱藏分頁控制，但不得省略任何資料。分頁只改變前端呈現，不改變既有清單 API 的資料與排序契約；刪除造成最後一頁沒有資料時，畫面會自動回到仍有資料的頁碼。
...
未使用邀請碼的每列在邀請碼旁提供「複製」與「刪除」操作。複製使用瀏覽器剪貼簿 API，成功後只顯示短暫的成功狀態；刪除由管理員 Bearer session 保護，後端以交易重新確認 `used=0` 後才刪除。已使用邀請碼刪除時回傳狀態衝突，不得刪除邀請碼、Channel 或 Notebook 綁定；已綁定資料仍由既有 Channel 刪除流程處理。
...
- 分頁避免大量清單在單一畫面中遺漏早期資料；複製與刪除操作放在邀請碼旁，降低管理者核對與清理成本。
