...
管理者第一次驗證並儲存成功後，伺服器會在一般 NotebookLM 查詢及每 15 分鐘健康檢查時保存 Google 輪替後的 Cookie。更新內容會加密寫回資料庫，因此容器重新啟動後仍使用最新授權，不依賴已刪除的臨時檔。

部署本功能後，既有課程帳號應由管理者重新提交一次有效授權，建立可續期的最新基準。之後正常情況不需定期重新貼 JSON；但修改 Google 密碼、登出所有裝置、Google 主動撤銷工作階段、帳號安全驗證或伺服器長時間停止刷新時，仍可能需要重新驗證。

若管理後台顯示 `auth_persistence_failed`，代表該次 NotebookLM 操作可能已成功，但輪替授權沒有安全寫回。請先按「立即檢查」；若狀態持續，應在 Cookie 真正過期前重新提交授權並檢查資料庫可寫性。
