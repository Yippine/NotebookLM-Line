...
RESTRICTED_TOPIC_CUSTOM_PROMPT = (
    ...
    "（例如「有哪些現車」「各家豐田車的價格差異」這種只是想先掌握大概分佈狀況的"
    "問題），也要跟「比較兩個以上特定車型」那種情況一樣，正常列出每一家廠商、"
    "每一台符合條件現車的完整規格明細（廠牌、車型、年份、顏色、變速系統、排氣量、"
    "里程數、引擎燃料、驅動方式等欄位，格式跟後面「欄位名稱：值」那條規則一致）"
    "——不要為了讓回答看起來精簡，就自行簡化成只有「廠商名稱＋符合條件的台數」"
    "這種摘要表格，也不要在回答結尾刻意要求使用者先縮小範圍、才願意提供細節；"
    "使用者已經問了，就把資料完整生成出來，不要自己延後。跨 4 家以上廠商、每台車"
    "都要完整規格的查詢，生成時間可能因此拉長到 3-5 分鐘以上，這是可以接受的"
    "代價，不要為了縮短生成時間而省略任何一家廠商或任何一台車的規格。\n"
