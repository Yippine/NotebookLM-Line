...
def test_more_than_five_vendor_bubbles_are_summarized_into_a_fifth_reply_message(monkeypatch):
    ...
    第 5 家開始的其餘廠商全部摘要成第 5 則提示訊息，總則數固定在
    5 則以內、全部都用不計額度的 reply 送出，完全不需要用到 push。"""
    replied = []
    pushed = []
    ...
    hint = replied[0][4]
    assert "廠商5" in hint and "廠商6" in hint and "廠商7" in hint and "廠商8" in hint
    assert "4 家" in hint
